...
use wgpu::{
    ...
    RenderPipelineDescriptor, ShaderStages, StencilFaceState, StencilState, StoreOp, Texture,
...
impl Renderer3D {
    ...
    pub fn update_camera(&mut self, camera: &PerspectiveCamera) {
        ...
        let flat: [f32; 16] = [
            vp[0][0], vp[1][0], vp[2][0], vp[3][0], vp[0][1], vp[1][1], vp[2][1], vp[3][1],
            vp[0][2], vp[1][2], vp[2][2], vp[3][2], vp[0][3], vp[1][3], vp[2][3], vp[3][3],
        ];
        self.queue
            .write_buffer(&self.camera_buffer, 0, bytemuck::cast_slice(&flat));
    }

    pub fn upload_instances(&self, transforms: &[Transform3D], colors: &[[f32; 4]]) -> Buffer {
        let instances: Vec<MeshInstance> = transforms
            .iter()
            .zip(colors.iter())
            .map(|(t, c)| {
                let m = t.to_matrix();
