    ...
    fn update(&mut self, world: &mut World, events: &mut EventBus, _dt: f64) {
        ...
        for (entity, pos) in world.query::<Position>() {
            let radius = if world.has_component::<CircleRadius>(entity) {
                world
                    .get_component::<CircleRadius>(entity)
                    .expect("entity has CircleRadius after has_component check")
                    .0
            } else {
            ...
            for other in nearby {
                if other.entity > obj.entity {
                    ...
                    if dx * dx + dy * dy <= combined_radius * combined_radius
                        && seen.insert((obj.entity, other.entity))
                    {
...
impl Default for DeathCleanupSystem {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Default for PlatformerSystem {
    fn default() -> Self {
        Self::new()
    }
}
