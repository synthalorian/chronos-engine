...
impl ScriptHook {
    ...
    pub fn function_name(&self) -> &str {
        match self {
            ScriptHook::Start => "on_start",
            ScriptHook::Update => "on_update",
            ScriptHook::Destroy => "on_destroy",
            ScriptHook::Collision => "on_collision",
            ScriptHook::Trigger => "on_trigger",
            ScriptHook::Input(_) => "on_input",
        }
    }
}
...
mod tests {
    ...
    fn test_script_hook_variants_and_names() {
        assert_eq!(ScriptHook::Start.function_name(), "on_start");
        assert_eq!(ScriptHook::Update.function_name(), "on_update");
        assert_eq!(ScriptHook::Destroy.function_name(), "on_destroy");
        assert_eq!(ScriptHook::Collision.function_name(), "on_collision");
        assert_eq!(ScriptHook::Trigger.function_name(), "on_trigger");
        assert_eq!(
            ScriptHook::Input("key_down".into()).function_name(),
            "on_input"
        );
    }
