...
fn rgba8_to_rgb8(rgba: &[u8]) -> Vec<u8> {
    rgba.chunks_exact(4).flat_map(|c| &c[..3]).copied().collect()
}
...
mod tests {
    ...
    fn test_import_mipmaps_disabled() {
        ...
        assert!(result.is_ok());
        let imported = result.unwrap();
        assert!(imported.mipmaps.is_empty(), "Mipmaps should be empty when disabled");
    }
    ...
    fn test_mipmaps_4x4() {
        ...
        assert_eq!(mipmaps.len(), 2, "4x4 should produce 2 mipmap levels (2x2, 1x1)");
    ...
    fn test_mipmaps_3x3_odd() {
        ...
        assert_eq!(mipmaps.len(), 2, "3x3 should produce 2 mipmap levels (2x2, 1x1)");
        assert_eq!(mipmaps[0].width, 2);
        assert_eq!(mipmaps[0].height, 2);
        assert_eq!(mipmaps[1].width, 1);
        assert_eq!(mipmaps[1].height, 1);
    ...
    fn test_mipmaps_max_levels_one() {
        ...
        assert_eq!(mipmaps.len(), 1, "max_levels=1 should produce only 1 mipmap");
    ...
    fn test_output_format_rgb8() {
        ...
        assert!(result.is_ok());
        let imported = result.unwrap();
        assert_eq!(imported.pixels.len(), 4 * 4 * 3, "RGB8 should have 3 bytes per pixel");
    }
    ...
    fn test_output_format_grayscale8() {
        ...
        assert!(result.is_ok());
        let imported = result.unwrap();
        assert_eq!(imported.pixels.len(), 4 * 4, "Grayscale8 should have 1 byte per pixel");
    }

    // Bonus: InvalidDimensions error for zero-size (simulated)
    #[test]
    fn test_error_display() {
        let err = ImageImportError::InvalidDimensions { width: 0, height: 5 };
        let msg = format!("{}", err);
        assert!(msg.contains("0x5"), "Error message should contain dimensions");
