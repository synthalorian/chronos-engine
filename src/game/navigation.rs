...
impl Pathfinder {
    ...
    fn reconstruct_path(
        &self,
        closed: &[(usize, usize, Option<(usize, usize)>)],
        end_x: usize,
        end_y: usize,
        terrain: &TerrainGrid,
        cell_size: f32,
    ...
    pub fn find_path(
        ...
        let mut closed: Vec<(usize, usize, Option<(usize, usize)>)> = Vec::new();
...
mod tests {
    ...
    fn navigation_path_traversal() {
        ...
        assert_eq!(path.next(), Some([1.0, 0.0, 1.0]));
        assert_eq!(path.next(), Some([2.0, 0.0, 2.0]));
        assert!(!path.is_complete());

        assert_eq!(path.next(), Some([3.0, 0.0, 3.0]));
        assert!(path.is_complete());
