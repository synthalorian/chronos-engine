...
pub struct EditorState {
    ...
    /// Whether recent-projects list has changed and needs persisting.
    pub recent_dirty: bool,

    /// Timestamp of the last auto-save so we don't spam disk I/O.
    pub last_auto_save: Option<std::time::Instant>,

    // ── Action request flags (set by panels, consumed by EditorApp) ──
    /// Request an undo operation on the next frame.
    pub undo_requested: bool,
    /// Request a redo operation on the next frame.
    pub redo_requested: bool,
    /// Request deletion of all selected entities.
    pub delete_selected_requested: bool,
    /// Request duplication of all selected entities.
    pub duplicate_selected_requested: bool,
    /// Request selection of all alive entities.
    pub select_all_requested: bool,
    /// Request the Save-As dialog.
    pub save_as_requested: bool,
    /// Request a fullscreen toggle.
    pub fullscreen_requested: bool,
    /// Request a layout reset.
    pub reset_layout_requested: bool,

    /// Request launching the engine with the current project.
