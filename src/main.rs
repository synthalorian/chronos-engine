...
fn chronos_company_demo() {
    ...
    let mut game = ChronosCompanyGame::new(config);
    game.new_game();

    println!("▶ New game started!");
    println!("  Squad size: {}", game.squad_entities.len());
    println!("  Starting gold: {}", game.state.gold);
    println!("  World seed: {}", game.config.world_seed);
    println!(
        "  Current screen: {:?}",
        game.screen_manager.current_state()
    );
    println!("  Camera target: {:?}", game.camera.target);
    println!("  Region: {}", game.state.current_region);
    println!();

    // Simulate 120 ticks (~2 seconds)
    println!("▶ Simulating 120 ticks...");
    for tick in 0..120 {
        game.tick(0.016);

        if tick % 30 == 0 {
            println!("  [{}] {}", tick, game.status_summary());
        }
    }
    ...
    // Test movement order
    println!("▶ Ordering squad to move...");
    let move_target = [25.0, 0.0, 25.0];
    game.order_move(move_target);
    println!("  Target: {:?}", move_target);

    // Test save/load
    println!("\n▶ Saving game to slot 1...");
    let save_id = game.save_game(1).unwrap();
    println!("  Saved to slot {}", save_id);

    println!("\n▶ Mutating state...");
    game.state.gold = 0;
    game.state.player_level = 99;
    println!("  Gold set to 0, Level set to 99");
    ...
    println!(
        ...
        game.encounter_manager.completed_count
    );

    // Test reputation
    println!("\n▶ Faction reputation:");
    game.reputation
        .modify_reputation(chronos_engine::game::factions::FactionId::CityWatch, 25);
    println!(
        "  City Watch: {}",
        game.reputation
            .get_value(chronos_engine::game::factions::FactionId::CityWatch)
    );

    // Final status
    println!("\n▶ Final status:");
    println!("  {}", game.status_summary());
    println!("  Play time: {:.1}s", game.state.total_play_time);
    println!("  Alive squad members: {}", game.squad_entities.len());
    println!("  Screen: {:?}", game.screen_manager.current_state());

    println!("\n═══ Chronos Company Demo Complete ═══");
}
    ...
    for frame in 0.. {
        ...
        for _ in 0..3 {
            if accumulator >= 0.016 {
                ...
                let pending = events.drain();
                for event in &pending {
                    if let Event::Collision(_a, b) = event {
                        let enemy_entity = world.entity_from_index(*b);
                        if world.entity_exists(enemy_entity) {
                            world.add_component(enemy_entity, Damage(25));
                        }
...
fn main() {
    ...
    {
        // Run the Phase 14 plugin-based demo (requires game feature)
        #[cfg(feature = "game")]
        chronos_engine::demo::run();

        #[cfg(not(feature = "game"))]
        {
            // Fallback demos when game feature is not available
            battle_demo();
            rts_scenario();
            bullet_hell_demo();
        }

        println!("\n✓ Chronos Engine is operational. Phase 14 demo complete.");
    }
}
