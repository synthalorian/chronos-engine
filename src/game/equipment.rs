...
impl StatBonus {
    ...
    /// Convenience alias — identical to [`new`].
    pub fn zero() -> Self {
        Self::new()
    }
...
impl EquipmentManager {
    ...
    pub fn equip(&mut self, data: EquipmentData) -> Option<EquipmentData> {
        ...
        std::mem::replace(&mut self.equipped[idx], Some(data))
    }

    /// Remove gear from a slot, returning it.
    pub fn unequip(&mut self, slot: EquipSlot) -> Option<EquipmentData> {
        std::mem::replace(&mut self.equipped[slot.as_index()], None)
    }
    ...
    pub fn total_bonuses(&self) -> StatBonus {
        ...
        for slot in self.equipped.iter() {
            if let Some(ref gear) = slot {
